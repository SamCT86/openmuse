...
## Known gaps
No OS sandbox, OS-backed master-key storage, full JSON Schema implementation, or production-ready human approval UI yet. Tool arguments are validated against a deliberately small object/required/properties/type subset; unsupported schema keywords fail closed. The local vault encrypts values at rest, but tools and key material still share the Python process. Use test data only.
